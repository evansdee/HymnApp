  ...
  const hymns = Object.keys(hymnObj).map(ele=>{
    return hymnObj[`${ele}`]
})

const catName = Object.keys(catHymn).map(ele=>(ele))

const catBody = Object.keys(catHymn).map(ele=>{
  return catHymn[`${ele}`]
})

  export{catBody,catName,hymns}